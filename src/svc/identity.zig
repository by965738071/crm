//! 身份与凭证业务：密码哈希/校验、登录防暴力计数、当前用户上下文类型。
//!
//! 命名为 identity 而非 auth，避免与框架 http_security/auth.zig 同名混淆：
//! 那是无状态共享密钥校验（Bearer/Basic/APIKey 与启动时配置的常量比对），
//! 本模块是 per-user 身份体系（哈希存储、账号锁定、角色语义），关注点不同。
